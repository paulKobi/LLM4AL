...
| Parameter | Value |
|---|---|
| Grid dimensions | 3 columns × 3 rows |
...
A position `(row, col)` is valid if and only if it lies within the grid bounds and the corresponding cell type is not `WALL`. All nine cells in this grid are within bounds; no cell is typed `WALL`. Implementations MUST NOT place the agent outside the grid boundary at any point during an episode.
...
```
s_0 = (row=1, col=0, door_status=LOCKED)
```
...
| `4` | `SUBMIT_KEY(k)` | Submit four-digit key code `k` | Only at `(1, 0)` |
...
```
A(s) = { 0, 1, 2, 3 }           if (row, col) ≠ (1, 0)
A(s) = { 0, 1, 2, 3, 4 }        if (row, col) = (1, 0)
```

The agent stands "in front of the door" when it occupies `(1, 0)` — the floor tile immediately west of the door tile at `(1, 1)`. This is the unique tile from which the door can be interacted with.

Implementations MUST NOT make `SUBMIT_KEY` available from any position other than `(1, 0)`. Implementations MUST raise an error or return an invalid-action signal if `SUBMIT_KEY` is issued from any other position. This is a hard interface constraint, not a no-op.
...
`SUBMIT_KEY(k)` is only reachable when `(row, col) = (1, 0)` (see Section 5.2). The door tile acted upon is always `(1, 1)`. Let `k_correct` denote the single correct key code for the episode (an implementation-defined constant in [0, 9999]).
...
**Case 1 — Agent at `(1, 0)`, `k ≠ k_correct`:**
...
**Case 2 — Agent at `(1, 0)`, `k = k_correct`:**
...
The door tile at `(1, 1)` behaves as follows depending on `door_status`. Because `door_status` can be toggled by `SUBMIT_KEY` in either direction, the traversability of the door tile may change multiple times within a single episode.
...
```
1.  Environment initialises to s_0 = (1, 0, LOCKED).
...
5. MUST raise an error or return an invalid-action signal if `SUBMIT_KEY` is issued from any position other than `(1, 0)`.
6. MUST set `door_status = UNLOCKED` when `k = k_correct` is submitted at `(1, 0)`, and MUST set `door_status = LOCKED` when any `k ≠ k_correct` is submitted at `(1, 0)`, regardless of the prior value of `door_status`.
...
| Goal position | (1, 2) |
| Door position | (1, 1) |
| Key submission position | (1, 0) |
| Lava positions | (0, 1) and (2, 1) |
